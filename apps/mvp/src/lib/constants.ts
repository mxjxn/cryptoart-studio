...
export const APP_DESCRIPTION: string =
  "Create NFT auctions, place bids, and collect digital art. A multi-modal marketplace for artists and collectors on Farcaster.";
...
export const APP_ACCOUNT_ASSOCIATION: AccountAssociation | undefined = {
  header:
    "eyJmaWQiOjQ5MDUsInR5cGUiOiJjdXN0b2R5Iiwia2V5IjoiMHgwRTQzYTJBQ2IyRkIzZDlCYWE1MjIyYjE4M2UwNDMyYkVlMmM2NUFCIn0",
  payload: "eyJkb21haW4iOiJjcnlwdG9hcnQuc29jaWFsIn0",
  signature:
    "IkPegubbCMgyek/MWXcv2KFFI5Wq6jUQFocWpBCh3V5mKbbKFvNuwrUBHTWoySM0atW0WglNvfUBmMr1tNj3zhw=",
};
...
export const STP_V2_CONTRACT_ADDRESS =
  "0x4b212e795b74a36B4CCf744Fc2272B34eC2e9d90" as const;
...
export const GALLERY_ACCESS_NFT_CONTRACT_ADDRESS =
  "0x96640349f9e87A2FE151d8114Acbc53D6e43CE7A" as const;
...
// For homepage OG image (curated featured listings):
// - Cache for 1 day (86400 seconds) since featured listings are curated
// - This prevents re-rendering more than once per day
// - no-transform prevents CDNs from modifying the image
export const OG_IMAGE_CACHE_CONTROL_HOMEPAGE =
  "public, max-age=86400, s-maxage=86400, stale-while-revalidate=86400, no-transform";
...
export const ADMIN_CONFIG = {
  ...
  farcasterUsername:
    process.env.NEXT_PUBLIC_ADMIN_FARCASTER_USERNAME ||
    process.env.ADMIN_FARCASTER_USERNAME ||
    "",

  // Primary admin FID - checks NEXT_PUBLIC_ prefix first (client), then server-only var
  fid: parseInt(
    process.env.NEXT_PUBLIC_ADMIN_FID || process.env.ADMIN_FID || "0",
    10,
  ),
} as const;
...
const parseAdditionalAdminAddresses = (): `0x${string}`[] => {
  ...
  const envValue =
    process.env.NEXT_PUBLIC_ADDITIONAL_ADMIN_ADDRESSES ||
    process.env.ADDITIONAL_ADMIN_ADDRESSES;
  if (!envValue) return [];
  ...
  if (!sessionStorage.getItem(debugKey)) {
    ...
    console.log("[Admin Config] Loaded values:", {
      ...
      envVars: {
        ...
        NEXT_PUBLIC_ADMIN_FARCASTER_USERNAME:
          process.env.NEXT_PUBLIC_ADMIN_FARCASTER_USERNAME,
      },
    });
  }
