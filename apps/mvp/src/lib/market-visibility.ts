...
export function filterListingsForMarketMode<T>(
  listings: T[] | undefined,
  mode: MarketBrowseMode
): T[] {
  if (!listings?.length) return [];
  return listings.filter((listing) =>
    isVisibleOnMarket(listing as Record<string, unknown>, mode)
  );
}

export function filterMarketSections<T extends { listings?: unknown[] }>(
  sections: T[],
  mode: MarketBrowseMode
): T[] {
  return sections
    .map((section) => ({
      ...section,
      listings: filterListingsForMarketMode(section.listings, mode),
    }))
    .filter((section) => (section.listings?.length ?? 0) > 0);
}
