...
const FollowersYouKnow: FC<FollowersYouKnowProps> = memo(({ user }) => {
  const { data, onEndReached, isFetchingNextPage } = useFollowersYouKnow({
    fid: user.fid,
    limit: 20,
  });
  ...
  return (
    <Page meta={{ title: `Users you know following ${user.displayName}` }}>
      <BorderedMainContent>
        <PageHeader
          hideCastButton
          footer={<FollowsHeader user={user} focusedTab="followersYouKnow" />}
        >
          <PageTitle>
            <BackButton />
        ...
        <FlatList
          ...
          emptyView={
            <DefaultEmptyListView
              message={`${user.displayName} doesn't have any followers you know.`}
            />
          }
        />
      </BorderedMainContent>
