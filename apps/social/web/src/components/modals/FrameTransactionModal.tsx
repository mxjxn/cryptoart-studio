...
type FrameTransactionModalProps = {
  tx: FrameTransaction;
  onClose: () => void;
  onSwitchWallet: () => void;
};
...
const FrameTransactionModal: React.FC<FrameTransactionModalProps> = React.memo(
  ({ tx, onClose, onSwitchWallet }) => {
    return (
            ...
            <div className="flex h-auto w-[368px] flex-col items-start justify-center rounded-lg border p-6 pb-4 bg-app border-default">
              ...
              >
                <ErrorBoundary
                  fallbackRender={() => {
                    return (
                      <FrameTransactionModalErrorBoundary
                        tx={tx}
                        onClose={onClose}
                      />
                    );
...
type FrameTransactionModalErrorBoundaryProps = {
  tx: FrameTransaction;
  onClose: () => void;
};

const FrameTransactionModalErrorBoundary: React.FC<
  FrameTransactionModalErrorBoundaryProps
> = ({ tx, onClose }) => {
  const navigateExternal = useExternalNavigate();
  const { frameDomain, frameUrl } = useFrame();
      ...
      <span className="flex flex-col items-center space-y-4">
        <span className="flex w-full flex-col items-center space-y-2 text-sm">
          <span className="text-faint">
            Failed to request transaction from mini app
          </span>
          <span
            dir="rtl"
        ...
        <div className="flex w-full flex-col space-y-1">
          ...
          >
            <span className="flex flex-row items-center">
              <span className="">{tx.actionText}</span>
...
type FrameTransactionModalContentProps = {
  tx: FrameTransaction;
  onClose: () => void;
  onSwitchWallet: () => void;
};

const FrameTransactionModalContent: React.FC<
  FrameTransactionModalContentProps
> = ({ tx, onClose, onSwitchWallet }) => {
  const { frameDomain, dismissAppError } = useFrame();
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      setLoading(true);
      // Deprecated: Frame transactions no longer supported
      ...
      if (apiError && apiError.reason === 'frame_application_error') {
        ...
        setLoadingError('Failed to load action from the mini app server.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const onTrxError = React.useCallback(
    (error: SendTransactionErrorType | SignTypedDataErrorType) => {
      ...
      tx.onFailure({
        transactionDataCorrelationId: trxData.correlationId,
        reason,
        message,
        details,
      });
      ...
      try {
        ...
        tx.onSuccess({
          ...
          result: {
            ...
            transactionChainId: transaction.params.chainId,
            transactionDataCorrelationId: trxData?.correlationId,
          },
        });
      } catch (e) {
  ...
  const stateChanges = React.useMemo(() => {
    ...
    return stateChanges.map((change) => change.humanReadableDiff);
  ...
  if (loading || !trxData || !trxData.transactionScan) {
    return (
...
FrameTransactionModal.displayName = 'FrameTransactionModal';
