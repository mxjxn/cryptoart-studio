...
import { useChannelFollowers, useDebouncedValue } from 'farcaster-client-hooks';
...
const ChannelFollowersPage: FC = memo(() => {
  const { channelKey } = useParams('channelFollowers');
  ...
  const { onEndReached, flatData, isFetchingNextPage, isPending } =
    useChannelFollowers({
      channelKey,
  ...
  return (
    <Page meta={{ title: `Followers of /${channelKey}` }}>
      <BorderedMainContent>
        <PageHeader
          hideCastButton
          dynamicFooterHeight
        ...
        <FlatList
          ...
          emptyView={<DefaultEmptyListView message="No followers found" />}
        />
      </BorderedMainContent>
    </Page>
  );
...
ChannelFollowersPage.displayName = 'ChannelFollowersPage';

export { ChannelFollowersPage };
