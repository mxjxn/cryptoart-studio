...
import { useGlobalTokenPrice } from '../../../../stores/tokenStore';
import { buildGloballyCachedTokenKey } from './buildGloballyCachedTokenKey';

const useGloballyCachedToken = ({
  fallback,
}: {
  fallback: ApiTokenLink;
}): ApiTokenLink => {
  const { addUsage, removeUsage } = useGlobalCacheUsage();
  const price = useGlobalTokenPrice({
    ca: fallback.ca,
  ...
  // If React Query doesn't find our token in the cache (for example, if it was stale and garbage collected),
  // we do not want to try to "fetch" the token (i.e. execute the function passed in as the second argument).
  ...
  // when the global token data has changed, which leaves us showing stale data. Let's revert to using `useQuery`
  // to restore the re-rendering behavior, but hard-code `enabled` to `false` so it never tries to run the fetch function.
  // We need to provide a value here, so we'll return the given token, but we would never expect this to run.
  // If no data is found in the cache, we would expect the `data` returned by `useQuery` to be `undefined`,
  // in which case we'll fall back to returning the given token. See https://github.com/merkle-manufactory/mobile/pull/754.
  ...
  const [mergedValue, setMergedValue] = useState<ApiTokenLink>(
  ...
  return merge(
    ...
    cachedValue,
    price
      ? { priceUsd: price.priceUsd, priceUpdatedAt: price.timestamp }
      : undefined,
  );
};

export { useGloballyCachedToken };
