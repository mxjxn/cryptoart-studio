...
const useEnableChannelNotifications = () => {
  ...
  return useCallback(
    async ({ channelKey }: { channelKey: string }) => {
      const revertEnableChannelNotificationsUpdate =
      ...
      try {
        ...
        const response = await apiClient.enableChannelNotifications({
          channelKey,
        });

        if (!response.data.result.success) {
          revertEnableChannelNotificationsUpdate();
        }
...
export { useEnableChannelNotifications };
